...
app.post('/home/pick_color', function (req, res) {
    ...
    var insert_statement = 'INSERT INTO favorite_colors(hex_value, name, color_msg);'; // Write a SQL statement to insert a color into the favorite_colors table
    var color_select = 'SELECT name FROM favorite_colors;'; // Write a SQL statement to retrieve all of the colors in the favorite_colors table
...
app.get('/team_stats', function (req, res) {
    var all_games = 'select * from football_games;'; // ALL the games in the season
    var wins = 'select COUNT(*) AS wins from football_games WHERE home_score > visitor_score;'; //winning games
    var losses = 'select COUNT(*) AS losses from football_games WHERE home_score < visitor_score;'; //losing games
    db.task('get-everything', task => {
        return task.batch([
            task.any(all_games),
            task.any(wins),
        ...
        .then(data => {
            res.render('pages/team_stats', {
                my_title: "Team Stats",
                result_1: data[0],
                result_2: data[1][0].wins,
                result_3: data[2][0].losses
            })
        })
        .catch(err => {
...
app.get('/player_info/post', function (req, res) {//WAS /player_info/player_select
    ...
    var query3 = "SELECT COUNT(*) AS player_games FROM football_games WHERE '" + player_id + "'=ANY(players)";
    db.task('get-everything', task => {
        return task.batch([
            task.any(query1),
            task.any(query2),
        ...
        .then(data => {
            res.render('pages/player_info', {
                my_title: "Selected Player Info",
                result_1: data[0],
                result_2: data[1],
                result_3: data[2][0].player_games
            })
        })
        .catch(err => {
            // display error message in case an error
